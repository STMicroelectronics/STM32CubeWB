...
struct zigbee_app_info {
  ...
  struct ZbZclClusterT *temp_meas_server;
};
...
void APP_ZIGBEE_LCD_DisplayInit(void)
{
    ...
    BSP_LCD_Clear(0,SSD1315_COLOR_BLACK);
    BSP_LCD_Refresh(0);
    UTIL_LCD_DisplayStringAt(0, LINE(0), (uint8_t *)"Temp_Meas SERVER", CENTER_MODE);
    UTIL_LCD_DisplayStringAt(0, LINE(4), (uint8_t *)"-40C/125C +/- 0.5C", CENTER_MODE);
    BSP_LCD_Refresh(0);
}
...
static void APP_ZIGBEE_CheckWirelessFirmwareInfo(void)
{
  ...
  if (SHCI_GetWirelessFwInfo(p_wireless_info) != SHCI_Success) {
   ...
   case INFO_STACK_TYPE_ZIGBEE_RFD:
    ...
    APP_DBG("Temperature measurement Server on Endpoint %d",SW1_ENDPOINT);
    APP_DBG("**********************************************************");
  }
} /* APP_ZIGBEE_CheckWirelessFirmwareInfo */
