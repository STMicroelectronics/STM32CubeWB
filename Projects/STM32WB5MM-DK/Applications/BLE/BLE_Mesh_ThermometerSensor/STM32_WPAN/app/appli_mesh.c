...
/**
* @brief  Timer callback for temperature measurement and publication over MESH 
* @param  void
* @retval void
*/ 
static void PublishTempTimerCb(void)
{
  /* Set the task in the scheduler for the next execution */
  UTIL_SEQ_SetTask( 1<<CFG_TASK_TEMP_PUB_REQ_ID, CFG_SCH_PRIO_0);

  return;
}

/**
* @brief  task for the BLE MESH, publish measured temperature over MESH 
* @param  void
* @retval void
*/ 
static void PublishTemp_Task()
{
  /** Modify temperature range to fit your usecase conditions : in this example Tmin=20 degres C, Tmax = 30 degres C **/
...
#endif
#ifdef THERMOMETER_NODE  
  /* Register the task for Temperature publication over MESH */
  UTIL_SEQ_RegTask( 1<< CFG_TASK_TEMP_PUB_REQ_ID, UTIL_SEQ_RFU, PublishTemp_Task );
  
  /* Create and Start timer for the task */
  HW_TS_Create(CFG_TIM_PROC_ID_ISR, &tempPublicationTaskTimer_Id, hw_ts_Repeated, PublishTempTimerCb);
  
  HW_TS_Start(tempPublicationTaskTimer_Id, TEMP_PUBLICATION_PERIOD); 
#endif /* THERMOMETER_NODE */
}
