 ...
 */
#if (P2P_SERVER1 != 0)
static const char a_LocalName[] = {AD_TYPE_COMPLETE_LOCAL_NAME , 'P', '2', 'P', 'S', 'R', 'X', '1'};
...
static TX_THREAD    thread_AdvCancelProcess;
static TX_SEMAPHORE sem_AdvCancelProcessSignal;
static TX_THREAD    ConnIntUpdateReqProcess;
static TX_SEMAPHORE ConnIntUpdateReqProcessSignal;

/* Private function prototypes -----------------------------------------------*/
static void thread_AdvCancel_entry(ULONG argument);
static void thread_HciUserEvtProcess_entry(ULONG thread_input);
static void thread_ConnIntUpdateReq_entry(ULONG argument);
...
void APP_BLE_Init(TX_BYTE_POOL* p_byte_pool)
{
  ...
  tx_byte_allocate(p_byte_pool, (VOID**) &p_pointer, DEMO_STACK_SIZE, TX_NO_WAIT);
   ...
   */
  tx_semaphore_create(&sem_AdvCancelProcessSignal, "sem_AdvCancelProcessSignal", 0);
  tx_byte_allocate(p_byte_pool, (VOID**) &p_pointer, DEMO_STACK_SIZE, TX_NO_WAIT);
  tx_thread_create(&thread_AdvCancelProcess,
                   "thread_AdvCancelProcess",
                   thread_AdvCancel_entry,
                   0,
                   p_pointer,
                   DEMO_STACK_SIZE,
                   16,
                   ...
                   TX_AUTO_START);
#if (L2CAP_REQUEST_NEW_CONN_PARAM != 0)
  tx_semaphore_create(&ConnIntUpdateReqProcessSignal, "ConnIntUpdateReqProcessSignal", 0);
  tx_byte_allocate(p_byte_pool, (VOID**) &p_pointer, DEMO_STACK_SIZE, TX_NO_WAIT);
  tx_thread_create(&ConnIntUpdateReqProcess,
                   "ConnIntUpdateReqProcess",
                   thread_ConnIntUpdateReq_entry,
                   0,
                   p_pointer,
                   DEMO_STACK_SIZE,
                   16,
      ...
      switch (p_blecore_evt->ecode)
      {
        ...
        case (ACI_GAP_NUMERIC_COMPARISON_VALUE_VSEVT_CODE):
          ...
          APP_DBG_MSG("     - numeric_value = %ld\n",
                      (unsigned long)((aci_gap_numeric_comparison_value_event_rp0 *)(p_blecore_evt->data))->Numeric_Value);
...
{
#if (L2CAP_REQUEST_NEW_CONN_PARAM != 0 )    
	tx_semaphore_put(&ConnIntUpdateReqProcessSignal);
#endif
...
static void thread_AdvCancel_entry(ULONG argument)
{
  ...
  for (;;) {
             tx_semaphore_get(&sem_AdvCancelProcessSignal, TX_WAIT_FOREVER);
             Adv_Cancel();
  }
}
...
static void Adv_Cancel_Req(void)
{
  ...
  tx_semaphore_put(&sem_AdvCancelProcessSignal);
...
static void thread_ConnIntUpdateReq_entry(ULONG argument)
{
  ...
  for (;;)
  {
    tx_semaphore_get(&ConnIntUpdateReqProcessSignal, TX_WAIT_FOREVER);
