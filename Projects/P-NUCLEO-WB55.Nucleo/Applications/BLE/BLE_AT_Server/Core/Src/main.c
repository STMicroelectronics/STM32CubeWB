...
extern char global_name[20];
extern uint8_t global_bdaddress[6];
extern float global_tx_power;
extern uint8_t global_tx_power_code;
extern uint8_t global_adv_type;
extern float global_adv_int_min;
extern float global_adv_int_max;
...
void lpuart_Receive_IT_cb(void);
...
int main(void)
{
  ...
  /* Init the at module */
  stm32wb_at_Init(&at_buffer[0], sizeof(at_buffer));

  printf("AT BLE server starts\n");
  
  /* Init buffer for AT module */
  ble_at_server_Init();
  
  
  
  strcpy(global_name, "AT server");
  ...
  global_adv_enable = 1;
  global_adv_type = 0;
  global_adv_int_min = CFG_MINI_CONN_ADV_INTERVAL_MIN;
  global_adv_int_max = CFG_MINI_CONN_ADV_INTERVAL_MAX;
  
  global_io_capability = CFG_IO_CAPABILITY;  
...
PUTCHAR_PROTOTYPE
{
  ...
  HW_UART_Transmit(hw_uart1, (uint8_t *)&ch, 1, 0xFFFF);
  return ch;
}
...
uint8_t stm32wb_at_ll_Transmit(uint8_t *pBuff, uint16_t Size)
{
  ...
  status = HW_UART_Transmit(hw_lpuart1, pBuff, Size, 0xFFFF);
  printf("  TX: %s",pBuff);
...
void stm32wb_at_ll_Async_receive(uint8_t new_frame)
{
  ...
  HW_UART_Receive_IT(hw_lpuart1, &AT_received_byte, 1, &lpuart_Receive_IT_cb );
...
void lpuart_Receive_IT_cb(void)
{
  stm32wb_at_Received(AT_received_byte);

  return;
}
uint8_t stm32wb_at_Process_rx_frame(char * str)
{
  ble_at_server_Process_rx_frame(str);

  return 0;
}

/* Dummy function to be compliant over different IDEs */
uint8_t stm32wb_at_client_Process_rx_frame(char *  str)
{
  return 0;
}
