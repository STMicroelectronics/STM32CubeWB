...
{
  Custom_APP_BLE_IDLE,
  Custom_APP_BLE_FAST_ADV,
  Custom_APP_BLE_LP_ADV,
  Custom_APP_BLE_ADV_Broadcast,
  Custom_APP_BLE_ADV_Broadcast_LP,
  Custom_APP_BLE_SCAN,
  Custom_APP_BLE_LP_CONNECTING,
  Custom_APP_BLE_CONNECTED_SERVER,
  Custom_APP_BLE_CONNECTED_CLIENT
} Custom_APP_BLE_ConnStatus_t;
...
typedef struct
{
  ...
  Custom_APP_BLE_ConnStatus_t Custom_Device_Connection_Status;
  BLE_App_Transfer_Req_Status_t ButtonTransferReq;
  uint8_t RX_Int_timer_Id;
  uint8_t Adv_Type;
  uint8_t Adv_Int_Mode;
  uint8_t Conn_Int_Mode;
  /* USER CODE END PTD_1 */
...
static void Conn_Int_Update_Req (void);
static void Custom_Adv_Request(Custom_APP_BLE_ConnStatus_t NewStatus);
/* USER CODE END PFP */
...
void APP_BLE_Init(void)
{
  ...
  BleApplicationContext.Custom_Device_Connection_Status = Custom_APP_BLE_IDLE;
  ...
  HW_TS_Create(CFG_TIM_NOTIFY_INT, &(BleApplicationContext.RX_Int_timer_Id), hw_ts_Repeated, Send_Data_Task);
  UTIL_SEQ_RegTask(1<<CFG_TASK_CONN_PARAM_UPDATE_ID, UTIL_SEQ_RFU, Conn_Int_Update_Req);

  BleApplicationContext.Adv_Type = ADV_TYPE;
  BleApplicationContext.Adv_Int_Mode = CFG_ADV_INT_MODE_FAST;
  BleApplicationContext.Conn_Int_Mode = CFG_CONN_INT_MODE_FAST;
    ...
    case HCI_LE_META_EVT_CODE:
    {
      ...
      switch (p_meta_evt->subevent)
      {
        ...
        case HCI_LE_CONNECTION_COMPLETE_SUBEVT_CODE:
        {
          ...
          BleApplicationContext.Custom_Device_Connection_Status = Custom_APP_BLE_CONNECTED_SERVER;
...
void APP_BLE_Key_Button1_Action(void)
{
  ...
  {
    if(BleApplicationContext.ButtonTransferReq!= BLE_APP_TRANSFER_REQ_OFF)
      {
        BleApplicationContext.ButtonTransferReq = BLE_APP_TRANSFER_REQ_OFF;
        HW_TS_Stop(BleApplicationContext.RX_Int_timer_Id);

        APP_DBG_MSG(" STOP sending notification to central device \n");
      }
      else
      {
        
        BleApplicationContext.ButtonTransferReq = BLE_APP_TRANSFER_REQ_ON;
        HW_TS_Start(BleApplicationContext.RX_Int_timer_Id, NOTIFY_TX_INT_MS);
        APP_DBG_MSG(" START sending notification to central device \n");
      }
  }   
}
...
void APP_BLE_Key_Button2_Action(void)
{
  ...
  if(BleApplicationContext.Device_Connection_Status != APP_BLE_CONNECTED_SERVER)
  {
    if(BleApplicationContext.Adv_Int_Mode == CFG_ADV_INT_MODE_FAST)
    {
      BleApplicationContext.Adv_Int_Mode = CFG_ADV_INT_MODE_LP;
    }
    else
    {
      BleApplicationContext.Adv_Int_Mode = CFG_ADV_INT_MODE_FAST;
...
void APP_BLE_Key_Button3_Action(void)
{
  if(BleApplicationContext.Device_Connection_Status != APP_BLE_CONNECTED_SERVER)
  {
    if(BleApplicationContext.Adv_Type == ADV_TYPE)
    {
      BleApplicationContext.Adv_Type = ADV_TYPE_2;
    }
    else
    {
      BleApplicationContext.Adv_Type = ADV_TYPE;
  ...
  {
    if(BleApplicationContext.Conn_Int_Mode == CFG_CONN_INT_MODE_FAST)
    {
      BleApplicationContext.Conn_Int_Mode = CFG_CONN_INT_MODE_LP;
    }
    else
    {
      BleApplicationContext.Conn_Int_Mode = CFG_CONN_INT_MODE_FAST;
    ...
    UTIL_SEQ_SetTask(1 << CFG_TASK_CONN_PARAM_UPDATE_ID, CFG_SCH_PRIO_0);
  }  
}
...
static void Adv_Update(void)
{
  if((BleApplicationContext.Adv_Type == ADV_TYPE) && (BleApplicationContext.Adv_Int_Mode == CFG_ADV_INT_MODE_FAST))
  {
    BleApplicationContext.Custom_Device_Connection_Status = Custom_APP_BLE_FAST_ADV;
    BleApplicationContext.Device_Connection_Status = APP_BLE_FAST_ADV;
    Custom_Adv_Request(Custom_APP_BLE_FAST_ADV);
  }
  else if((BleApplicationContext.Adv_Type == ADV_TYPE) && (BleApplicationContext.Adv_Int_Mode == CFG_ADV_INT_MODE_LP)) 
  {
    BleApplicationContext.Custom_Device_Connection_Status = Custom_APP_BLE_LP_ADV;
    BleApplicationContext.Device_Connection_Status = APP_BLE_LP_ADV;
    Custom_Adv_Request(Custom_APP_BLE_LP_ADV);
  }
  else if((BleApplicationContext.Adv_Type == ADV_TYPE_2) && (BleApplicationContext.Adv_Int_Mode == CFG_ADV_INT_MODE_FAST))
  {
    BleApplicationContext.Custom_Device_Connection_Status = Custom_APP_BLE_ADV_Broadcast;
    Custom_Adv_Request(Custom_APP_BLE_ADV_Broadcast);
  }
  else if((BleApplicationContext.Adv_Type == ADV_TYPE_2) && (BleApplicationContext.Adv_Int_Mode == CFG_ADV_INT_MODE_LP))
  {
    BleApplicationContext.Custom_Device_Connection_Status = Custom_APP_BLE_ADV_Broadcast_LP;
    Custom_Adv_Request(Custom_APP_BLE_ADV_Broadcast_LP);
  }
  else
  {
    APP_DBG_MSG("ADV update failed \n");    
  }
}
...
static void Custom_Adv_Request(Custom_APP_BLE_ConnStatus_t NewStatus)
{
  ...
  switch(NewStatus)
  {
    ...
    case Custom_APP_BLE_ADV_Broadcast:
      ret = aci_gap_set_discoverable(ADV_TYPE_2,
                                     ADV_INTERVAL_MIN,
                                     ADV_INTERVAL_MAX,
                                     CFG_BLE_ADDRESS_TYPE,
                                     ADV_FILTER,
    ...
    case Custom_APP_BLE_ADV_Broadcast_LP:
      ret = aci_gap_set_discoverable(ADV_TYPE_2,
                                     ADV_LP_INTERVAL_MIN,
                                     ADV_LP_INTERVAL_MAX,
                                     CFG_BLE_ADDRESS_TYPE,
  ...
  switch(BleApplicationContext.Custom_Device_Connection_Status)
  {
    case Custom_APP_BLE_FAST_ADV:
      if (ret != BLE_STATUS_SUCCESS)
      {
          APP_DBG_MSG("==>> Start FAST_IND Advertising Failed , result: %d \n\r", ret);
      }
      else
      {
          APP_DBG_MSG("==>> Success: Start FAST_IND Advertising \n\r");
      }
      break;
    ...
    case Custom_APP_BLE_LP_ADV:
      if (ret != BLE_STATUS_SUCCESS)
      {
          APP_DBG_MSG("==>> Start LP_IND Advertising Failed , result: %d \n\r", ret);
      }
      else
      {
          APP_DBG_MSG("==>> Success: Start LP_IND Advertising \n\r");
      }
      break;
    ...
    case Custom_APP_BLE_ADV_Broadcast:
      if (ret != BLE_STATUS_SUCCESS)
      {
          APP_DBG_MSG("==>> Start FAST_BROADCAST Advertising Failed , result: %d \n\r", ret);
      }
      else
      {
          APP_DBG_MSG("==>> Success: Start FAST_BROADCAST Advertising \n\r");
      }
      break;
    ...
    case Custom_APP_BLE_ADV_Broadcast_LP:
      if (ret != BLE_STATUS_SUCCESS)
        {
            APP_DBG_MSG("==>> Start LP_BROADCAST Advertising Failed , result: %d \n\r", ret);
        }
        else
        {
            APP_DBG_MSG("==>> Success: Start LP_BROADCAST Advertising \n\r");
        }
      break;
...
static void Conn_Int_Update_Req (void)
{
  ...
  if(BleApplicationContext.Conn_Int_Mode == CFG_CONN_INT_MODE_FAST)
  {
    ret = aci_l2cap_connection_parameter_update_req(
                                                 ...
                                                 CONN_INT_REQ,
                                                 CONN_INT_REQ,
                                                 L2CAP_PERIPHERAL_LATENCY, /*Latency*/
                                                 L2CAP_TIMEOUT_MULTIPLIER); /*Timeout_Multiplier*/
  }
  else
  {
        ret = aci_l2cap_connection_parameter_update_req(
