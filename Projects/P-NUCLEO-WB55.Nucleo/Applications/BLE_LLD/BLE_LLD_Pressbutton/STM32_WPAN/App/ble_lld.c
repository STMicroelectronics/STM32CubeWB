...
static ActionPacket halDummyAp;
...
/**
 * @brief Initializes the BLE Radio in HAL mode.
 *
 * Radio is not yet ready to communicate. The next step is to set the Network
 * ID or Access Address before Send or Receive Packets.
 */
uint8_t HAL_BLE_LLD_Init(void)
{
  return(sendCommand(HAL_BLE_LLD_INIT_CMDCODE, &bleparam_BLE_LLD_Packet));
}
...
{
  param_BLE_LLD_network_t *params = (param_BLE_LLD_network_t *)&bleparam_BLE_LLD_Packet;
  params->id = ID ;
  return(sendCommand(HAL_BLE_LLD_SETNETWORKID_CMDCODE, &bleparam_BLE_LLD_Packet));
}

/**
 * @brief Sends one packet without listening for an acknowledge.
 *
 * @param[in] channel Channel used for transmission
 * @param[in] wakeup_time Time before transmission start (us)
 * @param[in] txBuffer Data to transmit (header+length+payload)
 * @param[in] dataCaseTx DataRoutine that will be called after Tx

 * @retval SUCCESS_0 if success
 * @retval INVALID_PARAMETER_C0 if invalid parameters
 * @retval RADIO_BUSY_C4 if radio is busy
 */
uint8_t HAL_BLE_LLD_SendPacket(uint8_t channel,
                           uint32_t wakeup_time,
                           uint8_t* txBuffer,
                           dataCase_t dataCaseTx)
{
  /* Register storage for reply */
  actionPackets[0] = &halDummyAp;

  param_BLE_LLD_send_t *params = (param_BLE_LLD_send_t *)&bleparam_BLE_LLD_Packet;
  params->packetNumber =  dataRoutineParams.packetNumber;
  params->channel = channel;
  params->wakeupTime = wakeup_time;
  params->setCase.dataCase =  dataCaseTx;
  memcpy((params->txrxBuffer.txBuffer),txBuffer,txBuffer[1]+2);
  return(sendCommand(HAL_BLE_LLD_SENDPACKET_CMDCODE, &bleparam_BLE_LLD_Packet));
}

/**
 * @brief Sends one packet and listen for an acknowledge.
 *
 * @param[in] channel Channel used for transmission and reception
 * @param[in] wakeup_time Time before transmission start (us)
 * @param[in] txBuffer Data to transmit (header+length+payload)
 * @param[out] rxBuffer Buffer to store ACK data
 * @param[in] receive_timeout Timeout for ACK reception (us)
 * @param[in] dataCaseTxAck DataRoutine that will be called after ACK

 * @retval SUCCESS_0 if success
 * @retval INVALID_PARAMETER_C0 if invalid parameters
 * @retval RADIO_BUSY_C4 if radio is busy
 */
uint8_t HAL_BLE_LLD_SendPacketWithAck(uint8_t channel,
                                  uint32_t wakeup_time,
                                  uint8_t* txBuffer,
                                  uint8_t* rxBuffer,
                                  uint32_t receive_timeout,
                                  dataCase_t dataCaseTxAck)
{
  /* Register storage for reply */
  actionPackets[0] = &halDummyAp;
  halDummyAp.data = rxBuffer;
  ...
  params->setCase.dataCase = dataCaseTxAck;
  memcpy((params->txrxBuffer.txBuffer),txBuffer,txBuffer[1]+2);
  return(sendCommand(HAL_BLE_LLD_SENDPACKETWITHACK_CMDCODE, &bleparam_BLE_LLD_Packet));
}

/**
 * @brief Receives one packet without acknowledge transmission.
 *
 * @param[in] channel Channel used for reception
 * @param[in] wakeup_time Time before listening start (us)
 * @param[out] rxBuffer Buffer to store received data
 * @param[in] receive_timeout Timeout for data reception (us)
 * @param[in] dataCaseRx DataRoutine that will be called after reception
 *
 * @retval SUCCESS_0 if success
 * @retval INVALID_PARAMETER_C0 if invalid parameters
 * @retval RADIO_BUSY_C4 if radio is busy
 */
uint8_t HAL_BLE_LLD_ReceivePacket(uint8_t channel,
                              uint32_t wakeup_time,
                              uint8_t* rxBuffer,
                              uint32_t receive_timeout,
                              dataCase_t dataCaseRx)
{
  /* Register storage for reply */
  actionPackets[0] = &halDummyAp;
  halDummyAp.data = rxBuffer;

  param_BLE_LLD_receive_t *params = (param_BLE_LLD_receive_t *)&bleparam_BLE_LLD_Packet;
  params->packetNumber = dataRoutineParams.packetNumber;
  params->packetStopRx = dataRoutineParams.packetStopRx;
  params->channel = channel;
  params->wakeupTime = wakeup_time;
  params->receiveTimeout = receive_timeout;
  params->setCase.dataCase = dataCaseRx;
  return(sendCommand(HAL_BLE_LLD_RECEIVEPACKET_CMDCODE, &bleparam_BLE_LLD_Packet));
}

/**
 * @brief Receives one packet with acknowledge transmission.
 *
 * @param[in] channel Channel used for reception and transmission
 * @param[in] wakeup_time Time before listening start (us)
 * @param[out] rxBuffer Buffer to store received packet
 * @param[in] txBuffer Acknowledge to transmit
 * @param[in] receive_timeout Timeout for data reception (us)
 * @param[in] dataCaseRx DataRoutine that will be called TODO: complete
 *
 * @retval SUCCESS_0 if success
 * @retval INVALID_PARAMETER_C0 if invalid parameters
 * @retval RADIO_BUSY_C4 if radio is busy
 */
uint8_t HAL_BLE_LLD_ReceivePacketWithAck(uint8_t channel,
                                     uint32_t wakeup_time,
                                     uint8_t* rxBuffer,
                                     uint8_t* txBuffer,
                                     uint32_t receive_timeout,
                                     dataCase_t dataCaseRx)
{
  ...
  return(sendCommand(HAL_BLE_LLD_RECEIVEPACKETWITHACK_CMDCODE, &bleparam_BLE_LLD_Packet));
}
