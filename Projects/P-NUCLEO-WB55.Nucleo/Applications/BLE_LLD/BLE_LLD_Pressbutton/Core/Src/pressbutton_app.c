...
static void halSend(void);
static void halReceive(void);
...
static uint8_t pressrxBuffer[TXRX_BUF_SIZE];

/* data buffer ACK Tab to receive RX ACK after TX */
static uint8_t pressAckrxBuffer[TXRX_BUF_SIZE];

static ActionPacket pressPacket[ACTION_PACKET_NB];
...
{
  HAL_BLE_LLD_Init();
  HAL_BLE_LLD_SetNetworkID(PRESS_ID);
}

static void halSend(void)
{
  BLE_LLD_PRX_SetdataRoutineMultiOptions(PRESS_PACKET_NUMBER_TX,PRESS_PACKET_STOP_RX);
  HAL_BLE_LLD_SendPacketWithAck(PRESS_CHANNEL_1,
                            PRESS_WAKEUP,
                            presstxBuffer,
                            pressAckrxBuffer,
                            PRESS_RECEIVE_ACK,
                            DATA_ROUT_HAL_TXMULTIACK);
}

static void halReceive(void)
{
  BLE_LLD_PRX_SetdataRoutineMultiOptions(PRESS_PACKET_NUMBER_RX,PRESS_PACKET_STOP_RX);
  HAL_BLE_LLD_ReceivePacketWithAck(PRESS_CHANNEL_1,
                               PRESS_WAKEUP,
                               pressrxBuffer,
                               pressAcktxBuffer,
                               PRESS_RECEIVE,
                               DATA_ROUT_HAL_RXMULTIACK);
}
