...
struct zigbee_app_info {
  ...
  struct ZbZclClusterT *meterid_client_1;
};
...
static void APP_ZIGBEE_MeterId_Client_Read_COMPANY_NAME_ATTR(void){
  ...
  status = ZbZclReadReq(zigbee_app_info.meterid_client_1, &req, APP_ZIGBEE_MeterId_Read_Attr_cb, NULL);
...
static void APP_ZIGBEE_ConfigEndpoints(void)
{
  ...
  zigbee_app_info.meterid_client_1 = ZbZclMeterIdClientAlloc(zigbee_app_info.zb, SW1_ENDPOINT);
  assert(zigbee_app_info.meterid_client_1 != NULL);
  ZbZclClusterEndpointRegister(zigbee_app_info.meterid_client_1);
