...
static void APP_ZIGBEE_NwkForm(void)
{
  if ((zigbee_app_info.join_status != ZB_STATUS_SUCCESS) && (HAL_GetTick() >= zigbee_app_info.join_delay))
  {
    ...
    if (status != ZB_STATUS_SUCCESS) {
      APP_DBG("Warning Startup failed, M0_AND_M4_minus20db : value of 60 centimeter for Join plus or minus 40cm, then 8 meters for On OR Off attempting again after a short delay ");
    }
    ...
    if (status != ZB_STATUS_SUCCESS) {
      APP_DBG("Warning Startup failed, ZB_APS_IB_ID_SCAN_COUNT set to 1 attempting again after a short delay");
    }
    ...
    if (ZbNwkIfSetTxPower(zigbee_app_info.zb,"wpan0",updated_val_tx_power) != true) {
      APP_DBG("Warning Startup failed, ZbNwkIfSetTxPower set to minus 20 attempting again after a short delay");
      }
