...
static void APP_ZIGBEE_NwkForm(void)
{
  if ((zigbee_app_info.join_status != ZB_STATUS_SUCCESS) && (HAL_GetTick() >= zigbee_app_info.join_delay))
  {
    ...
    if (status != ZB_STATUS_SUCCESS) {
      APP_DBG("Warning setup failed, M0_AND_M4_minus20db : value of 60 centimeter for Join plus or minus 40cm, then 8 meters for On OR Off attempting again after a short delay");
    }
    
    uint8_t val8;
    val8 = 2;
    status = ZbApsSet(zigbee_app_info.zb, ZB_APS_IB_ID_SCAN_COUNT, &val8, sizeof(val8));
    if (status != ZB_STATUS_SUCCESS) {
      APP_DBG("Warning setup failed, ZB_APS_IB_ID_SCAN_COUNT set to 1 attempting again after a short delay");
    }
    ...
    config.channelList.list[0].channelMask = ((1 << CHANNEL1) | (1 << CHANNEL2) | (1 << CHANNEL3)) ; /*Channels in use */

    int updated_val_tx_power = -20;
    if (ZbNwkIfSetTxPower(zigbee_app_info.zb,"wpan0",updated_val_tx_power) != true) {
      APP_DBG("Warning setup failed, ZbNwkIfSetTxPower set to minus 20 attempting again after a short delay");
     }
    printf("Switching to minus 20 db\r\n");
...
static void APP_ZIGBEE_SW1_Process()
{
  ...
  APP_DBG("SW1 PUSHED (SENDING TOGGLE TO ZC)");
