 ...
 */
static void APP_ZIGBEE_App_Init(void){
  /* Initialize Zigbee Diagnostic Server parameters */
  APP_ZIGBEE_Diagnostic_Server_Init();
}

/**
 * @brief  Diagnostic server initialization
 * @param  None
 * @retval None
 */
static void APP_ZIGBEE_Diagnostic_Server_Init(void){
  APP_DBG("[DIAGNOSTIC] Diagnostic server init done!\n");  
} /* APP_ZIGBEE_Diagnostic_Server_Init */
...
static void APP_ZIGBEE_CheckWirelessFirmwareInfo(void)
{
  ...
  if (SHCI_GetWirelessFwInfo(p_wireless_info) != SHCI_Success) {
   ...
   case INFO_STACK_TYPE_ZIGBEE_RFD:
    ...
    APP_DBG("Diagnostics Server on Endpoint %d", SW1_ENDPOINT);
