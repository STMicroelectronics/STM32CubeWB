...
static struct custom_ls_command_rsp_t rsp;

enum ZclStatusCodeT
ZbZcl_custom_ls_ServerSendCommandRsp(struct ZbZclClusterT *clusterPtr, 
...
struct zigbee_app_info {
  ...
  struct ZbZclClusterT *custom_ls_server;
};
static struct zigbee_app_info zigbee_app_info;


/**
 * @brief  Custom cluster long string callback 
 * @param  cluster pointer, command req,addr info,arg
 * @retval None
 */
...
{
      char *msg = "ST Long String custom cluster.";
      /* We display the message received here */
      APP_DBG(cmd_req->string);
...
static void APP_ZIGBEE_CheckWirelessFirmwareInfo(void)
{
  ...
  if (SHCI_GetWirelessFwInfo(p_wireless_info) != SHCI_Success) {
   ...
   case INFO_STACK_TYPE_ZIGBEE_RFD:
    ...
    APP_DBG("Custom Long String Server on Endpoint %d",SW1_ENDPOINT);
    APP_DBG("**********************************************************");
  }
} /* APP_ZIGBEE_CheckWirelessFirmwareInfo */
