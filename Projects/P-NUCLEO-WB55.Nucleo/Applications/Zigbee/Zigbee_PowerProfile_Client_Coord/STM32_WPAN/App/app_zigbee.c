...
static void APP_ZIGBEE_PowerProfile_Client_Init(void);
static enum ZclStatusCodeT APP_ZIGBEE_PowerProfile_ProfileNotify_cb(struct ZbZclClusterT *cluster, struct ZbZclPowerProfSvrProfileRsp *notify, struct ZbZclAddrInfoT *srcInfo, void *arg);
static enum ZclStatusCodeT APP_ZIGBEE_PowerProfile_StateNotify_cb(struct ZbZclClusterT *cluster, struct ZbZclPowerProfSvrStateRsp *notify, struct ZbZclAddrInfoT *srcInfo, void *arg);
static enum ZclStatusCodeT APP_ZIGBEE_PowerProfile_GetPrice_cb(struct ZbZclClusterT *cluster, struct ZbZclPowerProfCliProfileReq *req, struct ZbZclAddrInfoT *srcInfo, void *arg);
...
struct zigbee_app_info {
  ...
  struct ZbZclClusterT *powerprofile_client_1;
};
static struct zigbee_app_info zigbee_app_info;

/* Power Profile cluster callbacks */
struct ZbZclPowerProfClientCallbacks PowerProfClientCallbacks = {
  .profile_notify = APP_ZIGBEE_PowerProfile_ProfileNotify_cb,
  .state_notify = APP_ZIGBEE_PowerProfile_StateNotify_cb, 
  .get_price =  APP_ZIGBEE_PowerProfile_GetPrice_cb,
};

/* Functions Definition ------------------------------------------------------*/

/**
 * @brief  Power Profile ProfileNotify callback
 * @param  cluster: ZCL cluster
 * @param  notify: Profile notification
 * @param  srcInfo: source address
...
/**
 * @brief  Power Profile StateNotify callback
 * @param  cluster: ZCL cluster
 * @param  notify: State notification
 * @param  srcInfo: source address
 * @param  arg: passed argument
 * @retval None
 */
static enum ZclStatusCodeT APP_ZIGBEE_PowerProfile_StateNotify_cb(struct ZbZclClusterT *cluster, struct ZbZclPowerProfSvrStateRsp *notify, struct ZbZclAddrInfoT *srcInfo, void *arg){
  ...
  APP_DBG("[POWER PROFILE] Power Profile state notification received.");
  APP_DBG("[POWER PROFILE] Power Profile state: 0x%02x.", notify->record_list[0].state);
  APP_DBG("[POWER PROFILE] Power Profile remote control: %d.\n", notify->record_list[0].remote_control);
...
static enum ZclStatusCodeT APP_ZIGBEE_PowerProfile_GetPrice_cb(struct ZbZclClusterT *cluster, struct ZbZclPowerProfCliProfileReq *req, struct ZbZclAddrInfoT *srcInfo, void *arg){
  ...
  struct ZbZclPowerProfCliPriceRsp rsp;
  ...
  status = ZbZclPowerProfClientPriceRsp(cluster, srcInfo, &rsp, NULL, NULL);
  if(status != ZCL_STATUS_SUCCESS){
    APP_DBG("[POWER PROFILE] Error sending Power Profile GetPrice response.");
    assert(0);
  }
 ...
 */
static void APP_ZIGBEE_App_Init(void){
  /* Initialize Zigbee Power Profile Client parameters */
  APP_ZIGBEE_PowerProfile_Client_Init();
} /* APP_ZIGBEE_App_Init */

/**
 * @brief  Power Profile client initialization
 * @param  None
 * @retval None
 */
static void APP_ZIGBEE_PowerProfile_Client_Init(void){
  APP_DBG("[POWER PROFILE] Power Profile client init done!\n");  
} /* APP_ZIGBEE_PowerProfile_Client_Init */
...
static void APP_ZIGBEE_ConfigEndpoints(void)
{
  ...
  /* Power Profile Client */
  zigbee_app_info.powerprofile_client_1 = ZbZclPowerProfClientAlloc(zigbee_app_info.zb, SW1_ENDPOINT, &PowerProfClientCallbacks, NULL);
  assert(zigbee_app_info.powerprofile_client_1 != NULL);
  ZbZclClusterEndpointRegister(zigbee_app_info.powerprofile_client_1);
...
static void APP_ZIGBEE_CheckWirelessFirmwareInfo(void)
{
  ...
  if (SHCI_GetWirelessFwInfo(p_wireless_info) != SHCI_Success) {
   ...
   case INFO_STACK_TYPE_ZIGBEE_RFD:
    ...
    APP_DBG("Power Profile Client on Endpoint %d",SW1_ENDPOINT);
    APP_DBG("**********************************************************");
  }
} /* APP_ZIGBEE_CheckWirelessFirmwareInfo */
