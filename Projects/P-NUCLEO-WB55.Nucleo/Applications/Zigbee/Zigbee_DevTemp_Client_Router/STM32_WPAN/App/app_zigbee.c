...
struct zigbee_app_info {
  ...
  struct ZbZclClusterT *device_temp_client;
};
...
static void APP_ZIGBEE_ConfigEndpoints(void)
{
  ...
  /* device temperature Client */
  zigbee_app_info.device_temp_client = ZbZclDevTempClientAlloc(zigbee_app_info.zb, SW1_ENDPOINT);
  assert(zigbee_app_info.device_temp_client != NULL);
  ZbZclClusterEndpointRegister(zigbee_app_info.device_temp_client);
...
static void APP_ZIGBEE_CheckWirelessFirmwareInfo(void)
{
  ...
  if (SHCI_GetWirelessFwInfo(p_wireless_info) != SHCI_Success) {
   ...
   case INFO_STACK_TYPE_ZIGBEE_RFD:
    ...
    APP_DBG("Device Temperature Client on Endpoint %d",SW1_ENDPOINT);
    APP_DBG("**********************************************************");
  }
} /* APP_ZIGBEE_CheckWirelessFirmwareInfo */
...
static void APP_ZIGBEE_SW1_Process()
{
  ...
  readReq.count = 3U;
  readReq.attr[0]=ZCL_DEV_TEMP_CURRENT ;
  readReq.attr[1]=ZCL_DEV_TEMP_MIN_TEMP;
  readReq.attr[2]=ZCL_DEV_TEMP_MAX_TEMP;
  
  APP_DBG("[DEV TEMP] Read Coordinator Temperature Attributes");
  if(ZbZclReadReq(zigbee_app_info.device_temp_client, &readReq, device_temp_read_cb,zigbee_app_info.device_temp_client) != ZCL_STATUS_SUCCESS)
  {
...
/**
 * @brief  Device Temperature read attribute callback
 * @param  readRsp: Read response 
  ...
  for(i=0;i<readRsp->count;i++)
  {
    if(readRsp->attr[i].status == ZCL_STATUS_SUCCESS)
    {
      ...
      case ZCL_DEV_TEMP_MIN_TEMP:
        ...
        APP_DBG("[DEV TEMP] Min recorded Temperature= %dC",min_temp);
        break;
      case ZCL_DEV_TEMP_MAX_TEMP:
        max_temp = pletoh16(readRsp->attr[i].value);
        APP_DBG("[DEV TEMP] Max recorded Temperature= %dC",max_temp);
        break;
      default:
        break;
      }
