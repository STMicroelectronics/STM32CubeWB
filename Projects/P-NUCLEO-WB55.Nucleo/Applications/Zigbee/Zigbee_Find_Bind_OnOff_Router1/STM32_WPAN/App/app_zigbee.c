...
#define SW1_ENDPOINT                                11U
#define IDENTIFY_MODE_TIME                          30U
...
struct zigbee_app_info {
  ...
  struct ZbZclClusterT *scenes_client;
};
...
static void APP_ZIGBEE_ConfigEndpoints(void)
{
  ...
  /* Scenes server */
  zigbee_app_info.scenes_client = ZbZclScenesClientAlloc(zigbee_app_info.zb, SW1_ENDPOINT);
  assert(zigbee_app_info.scenes_client != NULL);
  ZbZclClusterEndpointRegister(zigbee_app_info.scenes_client);
}
...
static void APP_ZIGBEE_NwkJoin(void)
{
  if ((zigbee_app_info.join_status != ZB_STATUS_SUCCESS) && (HAL_GetTick() >= zigbee_app_info.join_delay))
  {
    ...
    if (status == ZB_STATUS_SUCCESS) {
      ...
      BSP_LED_Off(LED_RED);
      BSP_LED_On(LED_BLUE);
      numberOfBindings = numOfBindings();
      APP_DBG("Binding entries created: %d", numberOfBindings);
      if (numberOfBindings >= 1U) {
...
static void APP_ZIGBEE_CheckWirelessFirmwareInfo(void)
{
  ...
  if (SHCI_GetWirelessFwInfo(p_wireless_info) != SHCI_Success) {
   ...
   case INFO_STACK_TYPE_ZIGBEE_RFD:
    ...
    APP_DBG("Scenes Client on Endpoint %d",SW1_ENDPOINT);
    APP_DBG("**********************************************************");
  }
}
...
/**
 * @brief  Button 1 handler. Sends a store scene command via binding if device is on a network.
 * @param  None
 * @retval None
 */
static void APP_ZIGBEE_SW1_Process()
{
  ...
  uint64_t epid = 0U;
  struct zcl_scenes_store_request_t req;
  ...
  req.groupId = SW1_GROUP_ADDR;
  req.sceneId = SW1_SCENE_ID;
  APP_DBG("SW1 PUSHED (Send Store Scene Command)");
  status = zcl_scenes_client_store_req(zigbee_app_info.scenes_client, &req, NULL, NULL);
  if (status != ZCL_STATUS_SUCCESS) {
    APP_DBG("ERROR, failed to send Store Scenes Command");
  }
}

/**
 * @brief  Button 2 handler. Sends a recall scene command via binding if device is on a network.
 * @param  None
 * @retval None
 */
static void APP_ZIGBEE_SW2_Process()
{
  uint8_t status = ZCL_STATUS_FAILURE;
  uint64_t epid = 0U;
  struct zcl_scenes_recall_request_t req;
  ...
  APP_DBG("SW2 PUSHED (Send Recall Scene Command)");
  req.dst = *ZbApsAddrBinding;
  req.groupId = SW1_GROUP_ADDR;
  req.sceneId = SW1_SCENE_ID;
  req.transition = 0;
  status = zcl_scenes_client_recall_req(zigbee_app_info.scenes_client, &req, NULL, NULL);
  if (status != ZCL_STATUS_SUCCESS) {
    APP_DBG("ERROR, failed to send Recall Scenes Command");
  }
}

/**
 * @brief  Button 3 handler. Sends an onOff toggle command via binding if device is on a network.
 * @param  None
 * @retval None
 */
static void APP_ZIGBEE_SW3_Process()
{
  uint8_t status = ZCL_STATUS_FAILURE;
  uint64_t epid = 0U;
  struct ZbApsAddrT dst = *ZbApsAddrBinding;
  ...
  APP_DBG("SW3 PUSHED (Send OnOff Toggle Command)");
  status = ZbZclOnOffClientToggleReq(zigbee_app_info.onoff_client, &dst, NULL, NULL);
  if (status != ZCL_STATUS_SUCCESS) {
    APP_DBG("ERROR, failed to send OnOff Toggle Command");
  }
}
