...
struct zigbee_app_info {
  ...
  struct ZbZclClusterT *custom_ls_client;
};
static struct zigbee_app_info zigbee_app_info;

/* Custom messages */
static char *message = "ST Long String custom cluster. "
...
static char *wrong_message = "A mismatched string.";
...
void APP_ZIGBEE_Init(void)
{
  ...
  /* Start the Zigbee stack on CPU2 side */
...
static void APP_ZIGBEE_ConfigEndpoints(void)
{
  ...
  /* Custom long string cluster Client */
  zigbee_app_info.custom_ls_client = ZbZcl_custom_ls_ClientAlloc(zigbee_app_info.zb, SW1_ENDPOINT);
  assert(zigbee_app_info.custom_ls_client != NULL);
  ZbZclClusterEndpointRegister(zigbee_app_info.custom_ls_client);
...
/**
 * @brief  Custom long string cluster client callback
...
static void APP_ZIGBEE_CheckWirelessFirmwareInfo(void)
{
  ...
  if (SHCI_GetWirelessFwInfo(p_wireless_info) != SHCI_Success) {
   ...
   case INFO_STACK_TYPE_ZIGBEE_RFD:
    ...
    APP_DBG("Custom Long String Client on Endpoint %d",SW1_ENDPOINT);
    APP_DBG("**********************************************************");
  }
} /* APP_ZIGBEE_CheckWirelessFirmwareInfo */
...
/**
 * @brief push button SW1 send command long string with matching message 
 *        
 * @param  None
 * @retval None
 */
...
/**
 * @brief push button SW2 send command long string with mismatching message 
 *        
 * @param  None
 * @retval None
 */
static void APP_ZIGBEE_SW2_Process()
{
  ...
  if (ZbZclSet_custom_ls_ClientCommand(zigbee_app_info.custom_ls_client,
                                       &dst,wrong_message, custom_ls_client_cb, NULL) != ZCL_STATUS_SUCCESS) 
                {
                   APP_DBG("Error, ZbZclSet_custom_ls_ClientCommand failed (SW1_ENDPOINT)");
                }  
 ...
 * @retval None
 */
void static APP_ZIGBEE_ProcessNotifyM0ToM4(void)
{
