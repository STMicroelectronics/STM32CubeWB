...
uint8_t l_BlinkCpt = 30;

	if (JoinerState & 0xF0) {
		// Slow Blinking on going
		BSP_LED_Toggle(LED1);
	}

	switch (JoinerState & 0x0F) {
		case 0 : /* Default */
		  BSP_LED_Off(LED1);

		  if (joinerIPv6IfUp) {
			  joinerIPv6IfUp = FALSE;
			  /* First set ifconfig down*/
#ifdef ENABLE_OPENTHREAD_CLI
                          indexReceiveChar = 15;
                          memcpy(CommandString, "ifconfig down\n\r", indexReceiveChar);
                          
                          Send_CLI_To_M0();
#else
                          
			  memset(ThreadCliCmdBuffer.cmdserial.cmd.payload, 0x0U, 255U);
			  memcpy(ThreadCliCmdBuffer.cmdserial.cmd.payload, "ifconfig down\n\r", 15);
...
#ifdef ENABLE_OPENTHREAD_CLI
                  indexReceiveChar = 13;
		  memcpy(CommandString, "ifconfig up\n\r", indexReceiveChar);
		...
		case 1 : /* ifconfig in progress */
		  joinerIPv6IfUp = TRUE;
                  JoinerState = 0xF2;
#ifdef ENABLE_OPENTHREAD_CLI
                  indexReceiveChar = 22;
		  memcpy(CommandString, "joiner start STM32WB\n\r", indexReceiveChar);
                  
                  Send_CLI_To_M0();
#else
                  memset(ThreadCliCmdBuffer.cmdserial.cmd.payload, 0x0U, 255U);
		  memcpy(ThreadCliCmdBuffer.cmdserial.cmd.payload, "joiner start STM32WB\n\r", 22);
...
#endif /* ENABLE_OPENTHREAD_CLI */
		  /* Start slow Blinking */
		  HW_TS_Start(joinerTimerID, (uint32_t)SLOW_BLINK_TIMEOUT);

		  break;
		case 2 : /* joining in progress */
			break;
		case 3 : /* joining over and OK */
			BSP_LED_On(LED1);
			JoinerState = 4;
			break;
		case 0x0F : /* joining Error case */
			HW_TS_Stop(joinerTimerID);
    		while(l_BlinkCpt--) {
				BSP_LED_Toggle(LED1);
				HAL_Delay(50U);
			  }
