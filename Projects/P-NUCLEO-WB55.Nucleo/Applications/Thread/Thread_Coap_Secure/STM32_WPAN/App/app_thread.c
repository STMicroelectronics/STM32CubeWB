...
/* USER CODE BEGIN PD */
#define C_RESOURCE_PROVISIONING    "provisioning"
#define C_RESOURCE_SECURE          "server_secure"
...
static void APP_THREAD_SendCoapSecureMsg(void);
...
/* USER CODE BEGIN PV */
static otCoapResource OT_RessourceSecure =        {C_RESOURCE_SECURE, APP_THREAD_CoapRequestHandler,"ResourceSecureContext", NULL};
static otCoapResource OT_RessourceProvisionning = {C_RESOURCE_PROVISIONING, APP_THREAD_ProvisioningReqHandler,"ProvisioningContext", NULL};
...
static void APP_THREAD_DeviceConfig(void)
{
  ...
  otCoapSecureSetPsk(NULL, mPsk, mPskLength, mPskId, mPskIdLength);
  ...
  if (error != OT_ERROR_NONE)
  {
    APP_THREAD_Error(ERR_THREAD_COAP_SECURE_START,error);
  }
  ...
  /* Add CoAP resource : OT_RessourceProvisionning */
  ...
  /* Add COAP Secure resource */
  error = otCoapSecureAddResource(NULL, &OT_RessourceSecure);
  if (error != OT_ERROR_NONE)
  {
    APP_THREAD_Error(ERR_THREAD_COAP_ADD_RESSOURCE,error);
  }
...
/**
 * @brief Callback function called when CoAP Secure connection is done
 *
 * @param None
 * @retval None
 */
static void APP_THREAD_CoapSecureConnected(bool aConnected, void *aContext)
{
  if (aConnected)
  {
      ...
      /* Ask to start the first transfer */
      UTIL_SEQ_SetEvt(EVENT_COAP_SECURE_CONNECT);
  }
  else
  {
    APP_DBG("CoAP Disconnected");
  }
}
...

{
  ...
  if ((otCoapMessageGetType(pMessage) == OT_COAP_TYPE_CONFIRMABLE) ||
      (otCoapMessageGetCode(pMessage) == OT_COAP_CODE_GET))
  {
    ...
    const otMessageInfo * pMessageInfo)
{
  ...
  do{
    ...
    error = otCoapSecureSendResponse(NULL, pOT_MessageResponse, pMessageInfo);
    if (error != OT_ERROR_NONE && pOT_MessageResponse != NULL)
    {
      otMessageFree(pOT_MessageResponse);
...
/**
 * @brief Send a CoAP Secure request with defined parameters.
 *
 * @param[in]  aCoapRessource   A pointer to a otCoapResource.
 * @param[in]  aCoapType        otCoapType.
 * @param[in]  aCoapCode        otCoapCode.
 ...
 * @retval none.
 */
static void APP_THREAD_CoapSecureSendRequest(otCoapResource* aCoapRessource,
    otCoapType aCoapType,
    otCoapCode aCoapCode,
    uint8_t* aPayload,
    uint16_t Size,
    otCoapResponseHandler aHandler)
{
  ...
  do{
    ...
    if ((aCoapType == OT_COAP_TYPE_CONFIRMABLE) || (aCoapCode == OT_COAP_CODE_GET))
    {
      error = otCoapSecureSendRequest(NULL,
          pOT_Message,
          aHandler,
          "COAP_SECURE_CONTEXT_CONFIRMABLE");
    }
    else
    {
      error = otCoapSecureSendRequest(NULL,
          pOT_Message,
          NULL,
          "COAP_SECURE_CONTEXT_NON_CONFIRMABLE");
    }
  }while(false);
  if (error != OT_ERROR_NONE && pOT_Message != NULL)
  {
...
static void APP_THREAD_SendCoapSecureMsg(void)
{
  ...
  APP_DBG("CoAP Secure connect to server");

  sockaddr.mPort = OT_DEFAULT_COAP_SECURE_PORT;
  memcpy(&sockaddr.mAddress, &OT_PeerAddress, sizeof(OT_PeerAddress));
  error = otCoapSecureConnect(NULL, &sockaddr, APP_THREAD_CoapSecureConnected, "ContextSecure");
  if (error != OT_ERROR_NONE)
  {
    APP_DBG("ERROR : otCoapSecureConnect failed!");
    APP_THREAD_Error(ERR_THREAD_COAP_SECURE_CONNECT,error);
  }

  UTIL_SEQ_WaitEvt(EVENT_COAP_SECURE_CONNECT);
