...
static void APP_THREAD_CoapRespHandler(void                * pContext,
    otMessage           * pMessage,
    const otMessageInfo * pMessageInfo,
    otError               result);
static void APP_THREAD_DataRespSend(otMessage        * pMessage,
    const otMessageInfo * pMessageInfo);
...
//  otError   error = OT_ERROR_NONE;
  ...
  APP_THREAD_CoapSendRequest(&OT_Ressource,
      ...
      OT_COAP_CODE_PUT,
      OT_IpAdressAllocated[(device_receiver - 1U)],
      NULL,
      &OT_Command,
      sizeof(OT_Command),
                                          ...
                                          const otMessageInfo * pMessageInfo)
{
  ...
  do
  {
    ...
    {
      APP_THREAD_DataRespSend(pMessage, pMessageInfo);
...
static void APP_THREAD_AllocateUnicastIpAddr(uint8_t boardId, uint8_t flagNewIpAdd)
{
  ...
  else
  {
    ...
    error=otIp6RemoveUnicastAddress(NULL,&OT_NetIfAddress.mAddress);
  }
}
