...
void Configure_USART(void)
{
  ...
  /* Set Baudrate to 115200 using APB frequency set to 64000000 Hz */
  /* Frequency available for USART peripheral can also be calculated through LL RCC macro */
  /* Ex :
      Periphclk = LL_RCC_GetUSARTClockFreq(Instance); or LL_RCC_GetUARTClockFreq(Instance); depending on USART/UART instance
  
      In this example, Peripheral Clock is expected to be equal to 64000000 Hz => equal to SystemCoreClock
  */
...
void UserButton_Init(void)
{
  ...
  /* Enable a rising trigger External line 4 Interrupt */
...
{
  /* Turn LED2 Off on User push-button (SW1) press (allow to restart sequence) */
  LED_Off();
}
