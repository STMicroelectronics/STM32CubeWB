                                ...
                                const mbedtls_ecp_point *P )
{
  ...
  R->Z.p[0] = 1;
  if (otPlatPkaEnable( ) == 0)
  {
    ret = MBEDTLS_ERR_ECP_HW_ACCEL_FAILED;
    goto cleanup;
  }

  otPlatPkaP256StartEccScalarMul( (const uint32_t*)m->p,
                                 (const uint32_t*)P->X.p,
                                 (const uint32_t*)P->Y.p );
  /* Wait for PKA end of operation, system will loop under below function until
    result is ready */
  otPlatEndOfOperation( );
  otPlatPkaP256ReadEccScalarMul( (uint32_t*)R->X.p, (uint32_t*)R->Y.p );
  otPlatPkaDisable( );
 cleanup:
  return ret;
}
